...
\[
\bm{z}_n^0 = \begin{pmatrix}
\mathrm{I}(\zeta_0 \leq x < \zeta_1) \\
\vdots \\
\mathrm{I}(\zeta_{K-1} \leq x < \zeta_K)
\end{pmatrix}.
\]
Regressing $y_n \sim \bm{z}_n^0$ produces a \emph{piecewise constant} fit, returning the average of training responses within each interval.
Indicator regressors give discontinuous fits with zero derivative. To obtain a piecewise linear fit, augment with
\[
\bm{z}_n^1 = \begin{pmatrix}
\mathrm{I}(\zeta_0 \leq x < \zeta_1)(x - \zeta_0) \\
\vdots \\
\mathrm{I}(\zeta_{K-1} \leq x < \zeta_K)(x - \zeta_{K-1})
\end{pmatrix}.
\]
Regressing $y_n \sim \bm{z}_n^0 + \bm{z}_n^1$ yields a separate first-order Taylor approximation within each segment.
Generalizing to degree $p$,
\[
\bm{z}_n^p = \begin{pmatrix}
...
Let’s decompose this target $\mathbb{E}\left[(\hat{f}(\bm{x}_{\mathrm{new}}) - y_{\mathrm{new}})^2\right]$. 
\begin{align*}
\mathbb{E}\left[(\hat{f}(\bm{x}_{\mathrm{new}}) - y_{\mathrm{new}})^2\right]
&= \mathbb{E}\left[(\hat{f}(\bm{x}_{\mathrm{new}}) - \bar{f}(\bm{x}_{\mathrm{new}}) + \bar{f}(\bm{x}_{\mathrm{new}}) - f^{\star}(\bm{x}_{\mathrm{new}}) + f^{\star}(\bm{x}_{\mathrm{new}}) - y_{\mathrm{new}})^2\right] \\
&= \underbrace{\mathbb{E}\left[(\hat{f}(\bm{x}_{\mathrm{new}}) - \bar{f}(\bm{x}_{\mathrm{new}}))^2\right]}_{\text{``variance''}} + \underbrace{\mathbb{E}\left[(\bar{f}(\bm{x}_{\mathrm{new}}) - f^{\star}(\bm{x}_{\mathrm{new}}))^2\right]}_{\text{``bias''}} + \underbrace{\mathbb{E}\left[(f^{\star}(\bm{x}_{\mathrm{new}}) - y_{\mathrm{new}})^2\right]}_{\text{``irreducible error''}}.
\end{align*}
Define $M_{xx} = \EE [\bx^\top \bx]$ and $M_{xy} = \EE[\bx^\top y]$. Then $\bbeta = M_{xx}^{-1}M_{xy}$. By CLT,
\[
\hat{\bbeta} - \bbeta^*
= \left(\frac{1}{N} \sum_{n=1}^{N} \bm{x}_n \bm{x}_n^\top\right)^{-1} \frac{1}{N} \sum_{n=1}^{N} \bm{x}_n (y_n - \bm{x}_n^\top \bm{\beta}^*), \quad 
...
\subsubsection{Fundamentals}
Let us now turn to classification problems. Like regression, we assume we get IID observations $z_n = (\bm{x}_n, y_n)$, but now with the difference that $y$ takes one of a set of unordered distinct values, which I will call $\mathcal{Y}$. The simplest case is where $y$ takes one of two values, which I will sometimes call ``binary classification''. As before, we wish to use $\bm{x}$ to guess what $y$ is. As we will see shortly, there are more choices to what $f$ should even be for classification than there are for regression.

Ultimately, we need a mapping $\bm{x} \mapsto \mathcal{Y}$, so we might take $\hat{y}(\bm{x}) \in \mathcal{Y}$. Necessarily, the loss function must take in two values in $\mathcal{Y}$---the guess and the truth---and return a real number. Formally, $\mathscr{L}: \mathcal{Y} \times \mathcal{Y} \mapsto \mathbb{R}$, which can be fully represented as a $|\mathcal{Y}| \times |\mathcal{Y}|$ matrix.
...
\begin{align*}
\mathbb{E}_{\mathbb{P}(\bm{x},y)}\left[\mathscr{L}_{\text{ZO}}(\hat{y}, y)\right] 
&= \sum_{c \in \mathcal{Y}} \mathrm{I}(\hat{y} = c)\, \mathbb{E}_{\mathbb{P}(\bm{x},y)}\left[\mathrm{I}(y \neq c)\right] \\
&= \sum_{c \in \mathcal{Y}} \mathrm{I}(\hat{y} = c)\bigl(1 - \mathbb{P}(y = c, \bm{x})\bigr) \\
&= 1 - \sum_{c \in \mathcal{Y}} \mathrm{I}(\hat{y} = c)\bigl(1 - \mathbb{P}(y = c \mid \bm{x})\bigr)\mathbb{P}(\bm{x}).
\end{align*}
Since $\mathbb{P}(\bm{x})$ does not depend on $c$, the best choice for $\hat{y}$ is
\begin{equation*}
\hat{y}^*(\bm{x}) = \operatorname*{argmax}_{c \in \mathcal{Y}} \mathbb{P}(y = c \mid \bm{x}).
\end{equation*}

The natural generalization of zero--one loss and functions that return categories is a function that returns a probability. For binary regression with $y \in \{0, 1\}$, we can take $\pi(\bm{x})$ to approximate $\mathbb{P}(y = 1 \mid \bm{x})$, and so take $\pi: \bm{x} \mapsto [0, 1]$. 

\paragraph{Cross Entropy Loss}
If $\pi(\bm{x}) \in (0, 1)$ then we can in fact use other losses. One common choice is the negative log likelihood:
\begin{align*}
...
&= -y f - \log(1 + \exp(f)),
\end{align*}
And if we further take $f(\bm{x}) = \bm{\beta}^{\top} \bm{x}$, we get
\begin{equation*}
\mathscr{L}_{\text{CE}}(\operatorname{Expit}(\bm{\beta}^{\top}\bm{x}), y) = -y \bm{\beta}^{\top} \bm{x} - \log(1 + \exp(\bm{\beta}^{\top}\bm{x})),
\end{equation*}
which is \textbf{logistic regression}.

\paragraph{Generative modeling}
We have noted that the optimal classifier takes the form of the maximum of $\mathbb{P}(y = c \mid \bm{x})\mathbb{P}(\bm{x})$. In our reasoning above we ignored $\mathbb{P}(\bm{x})$ since it does not depend on $c$, and modeled $\mathbb{P}(y = c \mid \bm{x})$, e.g.\ using the expit function and linear models.
However, we could condition in the other direction to write
\begin{equation*}
\hat{y} = \operatorname*{argmax}_{c} \mathbb{P}(y = c \mid \bm{x})\mathbb{P}(\bm{x}) = \operatorname*{argmax}_{c} \mathbb{P}(\bm{x} \mid y = c)\mathbb{P}(y = c).
\end{equation*}
...
\end{equation*}
We can then attempt to model the conditional density $\mathbb{P}(\bm{x} \mid y = c)$ by forming density estimates the sets of points $\mathcal{X}_c := \{\bm{x}_n : y_n = c\}$. Density estimation is difficult in high dimensions---arguably more difficult than estimating good decision boundaries---but generative modeling can be particularly effective in low dimensions, or when something special is known about the structure of $\mathbb{P}(\bm{x} \mid y = c)$.

Note that the estimation $\mathbb{P}(\bm{x} \mid y = c)$ does not admit a ready formulation as a direct minimization of classification error. Estimation of densities is typically considered an \textbf{``unsupervised learning''} problem for this reason, even though here we would use the unsupervised learning estimate in service of a supervised learning problem.

\subsubsection{Support Vector Machines}
Suppose we have binary classification data $(\bm{x}_1, y_1), \ldots, (\bm{x}_N, y_N)$, where $y_n \in \{-1, 1\}$, and we want to find a linear decision boundary:
\begin{equation*}
\hat{y}_n = \operatorname{sign}(\bm{x}_n^{T} \hat{\bm{\beta}}).
\end{equation*}
Such a vector $\hat{\bm{\beta}}$ would be normal to a hyperplane that separates our predictions.
...
\begin{equation*}
\hat{\bm{\beta}}_{\text{hard}} = \operatorname*{argmax}_{\|\bm{\beta}\|_2 = 1,\, \gamma} \gamma \quad \text{subject to} \quad y_n \bm{x}_n^{T} \bm{\beta} \geq \gamma \quad \forall n = 1, \ldots, N.
\end{equation*}
\begin{prop}
\begin{equation*}
\hat{\bm{\beta}}_{\text{hard}} = \operatorname*{argmin}_{\bm{\beta}} \|\bm{\beta}\|_2 \quad \text{subject to} \quad y_n \bm{x}_n^{T} \bm{\beta} \geq 1 \quad \forall n = 1, \ldots, N.
\end{equation*}
\end{prop}

\paragraph{Soft Margin SVM}
The hard margin SVM is restricted to linearly separable data. We now introduce a soft margin (linear) SVM, which trades separability with $\|\bm{\beta}\|_2$:
\begin{equation*}
\hat{\bm{\beta}}_{\text{soft}}(\lambda) = \operatorname*{argmin}_{\bm{\beta}} \sum_{n=1}^{N} \max(1 - y_n \bm{x}_n^{T} \bm{\beta},\, 0) + \lambda \|\bm{\beta}\|_2^2.
\end{equation*}
This way, we may allow classification (or margin) mistakes for the purpose of reducing $\|\bm{\beta}\|_2^2$. This brings us to the familiar form of minimizing loss plus penalty.

\paragraph{Connection to hard margin SVM}
To see how soft margin SVM relates to hard margin, add slack variable $\xi_n \geq \max(1 - y_n \bm{x}_n^{T} \bm{\beta},\, 0)$ and observe that
\begin{equation*}
\hat{\bm{\beta}}_{\text{soft}}(\lambda) = \operatorname*{argmin}_{\bm{\beta}} \|\bm{\beta}\|_2^2 + \frac{1}{\lambda} \sum_{n=1}^{N} \xi_n \quad \text{subject to} \quad y_n \bm{x}_n^{T} \bm{\beta} \geq 1 - \xi_n,\; \xi_n \geq 0 \quad \forall n = 1, \ldots, N.
\end{equation*}

We also divided by $\lambda$. From this point we may intuitively believe that, for linearly separable data, $\hat{\bm{\beta}}_{\text{hard}}$ is obtained as the limit of $\hat{\bm{\beta}}_{\text{soft}}(\lambda)$ as $\lambda \to 0^{+}$, since the slack variables $\xi_1, \ldots, \xi_N$ are pushed to zero.

\paragraph{Interpretation as penalty for margin mistakes}
The slack variable $\xi_n$ is nonzero if we make a margin mistake on $(\bm{x}_n, y_n)$. It is greater than one if we make a classification mistake. The sum $\sum_{n=1}^{N} \xi_n$ quantifies the total over all margin mistakes, which we can control by adjusting the penalty $\lambda$.

\paragraph{Kernels}
Using convex optimization theory, one may show that the soft margin SVM solution will satisfy
\begin{equation*}
\hat{\bm{\beta}}_{\text{soft}} = \sum_{n=1}^{N} \alpha_n y_n \bm{x}_n \quad ; \quad \text{where} \quad \alpha_n \neq 0 \text{ if and only if } y_n \bm{x}_n^{T} \hat{\bm{\beta}}_{\text{soft}} \leq 1.
\end{equation*}
If we used an expressive feature transformation $\varphi(\bm{x}_n) \in \mathbb{R}^{Q}$, the soft margin SVM $\hat{\bm{\beta}}_{\text{kernel}} \in \mathbb{R}^{Q}$ would satisfy
\begin{equation*}
\hat{\bm{\beta}}_{\text{kernel}} = \sum_{n=1}^{N} \alpha_n y_n \varphi(\bm{x}_n).
\end{equation*}
The prediction on a new observation would be
\begin{equation*}
y = \operatorname{sign}\!\left(\varphi(\bm{x})^{T} \hat{\bm{\beta}}_{\text{kernel}}\right) = \sum_{n=1}^{N} \alpha_n y_n \varphi(\bm{x})^{T} \varphi(\bm{x}_n)=: \sum_{n=1}^{N} \alpha_n y_n k(\bm{x},\bm{x}_n).
\end{equation*}
...
\end{equation}
where $t \in T'$ means $t$ is a leaf node of $T'$, $n \in t$ means $\bm{x}_n$ is assigned to leaf node $t$, and $|T'|$ counts the number of leaf nodes.
